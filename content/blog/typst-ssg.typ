...
Larger scripts go in their own file with `#script("counter.ts")`, which compiles
to a sibling `.js` and links it in the head. Each file is transpiled on its own
by Node's built-in type stripping, so there is no bundler, but also no cross-file
imports.
